...
import { LeadStatus, QualificationLevel, Gender, MaritalStatus } from '../enums'
...
export const CreateLeadSchema = z.object({
  ...
  fatherName:  z.string().trim().min(2, 'Father name must be at least 2 characters'),
  ...
  passingYear:     z.number().int().min(1990).max(new Date().getFullYear()).optional(),
...
export const LeadListQuerySchema = z.object({
  ...
  sortBy:       z.enum(['createdAt', 'studentName', 'status', 'nextFollowUpAt']).default('createdAt'),
