...
function checkR2Config(): string | null {
  ...
  if (!config.r2BucketName || config.r2BucketName === "local") return "R2_BUCKET_NAME is not set";
  if (!config.r2PublicUrl || config.r2PublicUrl.startsWith("http://localhost")) return "R2_PUBLIC_URL is not set to a public URL";
  return null;
}
