...
export async function leadRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      for (const r of records) {
        if (
          r.level &&
          !VALID_ACADEMIC_LEVELS.has(r.level as QualificationLevel)
        ) {
          return reply.status(400).send({
            success: false,
            error: {
              code: "INVALID_INPUT",
              message: `Invalid academic level: ${r.level}`,
            },
          });
        }
      }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const { documentTypeId, fileUrl, fileName } = request.body as {
    ...
    async (request, reply) => {
      ...
      if (
        !canUpdateLead(
          ...
          {
            id: leadForAuth.id,
            assignedToId: leadForAuth.assignedToId ?? null,
            createdById: leadForAuth.createdById,
            branchId: leadForAuth.branchId,
            status: leadForAuth.status,
          },
        )
      ) {
        return reply
        ...
        if (
          ...
          return reply.status(409).send({
            ...
            error: {
              ...
              message:
                "That file number is already used by another application. Please choose a different one.",
            },
          });
        }
        throw err;
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const fsmResult = transitionLead(
      ...
      if (!fsmResult.success) {
        return reply.status(400).send({
          ...
          error: {
            ...
            message: `Cannot confirm from status ${lead.status}: ${fsmResult.error.message}`,
          },
        });
      }
